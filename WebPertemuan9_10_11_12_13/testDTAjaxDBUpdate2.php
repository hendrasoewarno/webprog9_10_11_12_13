...
<table id="example" class="display" style="width:100%">
        <thead>
            <tr>
                <th>RoomId</th><th>Lantai</th><th>JenisKamar</th><th>Status</th><th>Keterangan</th><th>Action</th>
            </tr>
        </thead>
</table>
</div>
...
var table = $('#example').DataTable( {
	...
	columns: [
        { data: 'RoomId' }, { data: 'Lantai' },  { data: 'JenisKamar' },  { data: 'Status' },  { data: 'Keterangan' },
...
if (isset($_REQUEST["flag"])) {
	if ($_REQUEST["flag"]=="show") {
		...
		$sqlData = "SELECT RoomId, Lantai, JenisKamar, case when Status=1 Then 'Aktif' else 'Non-Aktif' end as Status, Keterangan FROM room WHERE (RoomId LIKE :search or JenisKamar Like :search) LIMIT $length OFFSET $start";
		$data = array();
